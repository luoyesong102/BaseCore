...
namespace SAAS.FrameWork.Util.Guid
{
    /// <summary>
    ...
    public class SnowflakeObj
    {
        ...
        public static void SetMachineId(long machineId)
        {
            if (machineId >= 0)
            {
                ...
                {
                    SnowflakeObj.machineId = machineId;
                    return;
                }
            }
        ...
        {            
            ...
            if (datacenterId >= 0)
            {
                ...
                {
                    SnowflakeObj.datacenterId = datacenterId;
                    return;
                }
            }
        ...
        public static long GetId()
        {
            ...
            {
                long timestamp = GetTimestamp();
                if (SnowflakeObj.lastTimestamp == timestamp)
                { //同一微妙中生成ID
                    sequence = (sequence + 1) & sequenceMask; //用&运算计算该微秒内产生的计数是否已经到达上限
                    if (sequence == 0)
                    {
                        //一微妙内产生的ID计数已达上限，等待下一微妙
                        timestamp = GetNextTimestamp(SnowflakeObj.lastTimestamp);
                    }
                }
                else
                {
                ...
                SnowflakeObj.lastTimestamp = timestamp; //把当前时间戳保存为最后生成ID的时间戳

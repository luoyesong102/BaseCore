        ...
        class ScopeCache : IDisposable
        {
            ...
            public void Dispose()
            {
                lock (ioc)
                {
                    ...
                    {                        
                        throw new Exception($"did not call IServiceScope.Dispose in order[SAAS.FrameWork.Util.Ioc,{ nameof(Ioc) }.cs,Line {GetFileLineNumber()}]");
